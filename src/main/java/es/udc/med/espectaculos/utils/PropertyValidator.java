...
public final class PropertyValidator {
	...
	public static void validateEvent(Evento event) throws InputValidationException {
		if (event.getNombreEvento() == null || event.getNombreEvento().equals("")) {
		...
		if (event.getFechaInicioEvento().get(Calendar.DAY_OF_MONTH) < fechaActual.get(Calendar.DAY_OF_MONTH)) {
			throw new InputValidationException("Día anterior al actual");
		}
		if (event.getFechaInicioEvento().get(Calendar.MONTH) < fechaActual.get(Calendar.MONTH)) {
			throw new InputValidationException("Mes anterior al actual");
		}
		if (event.getFechaInicioEvento().get(Calendar.YEAR) < fechaActual.get(Calendar.YEAR)) {
